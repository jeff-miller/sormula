...
public class Table<R> implements TypeTranslatorMap, TransactionListener
{
     ...
     * is not already in database.
     * <p>  
     * If row does not exist in database, inserts a row into a table that was defined 
     * with an identity column but insert does not use auto generated key for 
     * identity column. This allows you to insert a row into a table when identity 
     * column value is obtained from the row object.
     * 
     * @param row row to save
     * @return count of rows affected
     * @throws SormulaException if error
     * @since 4.1
     */
    public int saveNonIdentity(R row) throws SormulaException
    {
        return new SaveOperation<R>(this, false).modify(row);
    }
    ...
    /**
     * Uses {@link SaveOperation} to update an existing rows or insert rows if they
     * are not already in database. Inserts do not use autogenerated keys.
     * 
     * @param rows collection of new and/or existing rows to save (may be mixture of new and existing)
     * @return count of rows affected
     * @throws SormulaException if error
    ...
    {
        if (rows.size() > 0) return new SaveOperation<R>(this, false).modifyAll(rows);
        else return 0;
    }
     ...
     * Saves a collection of rows in batch mode.  Related cascades are batched also.
     * See limitations about batch saves in {@link ModifyOperation#setBatch(boolean)}.
     * Inserts do not attempt to get autogenerated keys from database.
     * 
     * @param rows collection of new and/or existing rows to save (may be mixture of new and existing)
     * @return count of rows affected
     * @throws SormulaException if error
    ...
    public int saveNonIdentityAllBatch(Collection<R> rows) throws SormulaException
    {
        if (rows.size() > 0)
        {
            SaveOperation<R> operation = new SaveOperation<R>(this, false);
            operation.setBatch(true);
            return operation.saveAll(rows);
        }
        else
